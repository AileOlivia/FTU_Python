...
def get_args():
    ...
    parser.add_argument('--model_name', type=str, default='resnext50', help='log、ckpt name')
...
os.system(f'mkdir ../input/ckpt-frank/{opts.model_name}')
os.system(f'mkdir logs/{opts.model_name}')
...
#some more things 
logger=CSVLogger_my(['epoch','train_loss','val_loss','dice_coef'], f"./logs/{opts.model_name}/log_fold{opts.fold}.csv")
...
for epoch in range(opts.epochs):
    ...
    if metric_this_epoch>best_metric:
        torch.save(model.state_dict(),f'../input/ckpt-frank/{opts.model_name}/fold{opts.fold}.pth')
